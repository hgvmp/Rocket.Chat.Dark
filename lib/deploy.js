 ...
 * @param {Function} 	done    Callback.
 * @param {bool} 		  minify 	Check if the minified file should be used
 */
function deployCSS(cssFile, url, headers, done, minify = false) {
	let fileExt = minify ? '.min.css' : '.css';
	let data = {
		value: fs.readFileSync(cssFile + fileExt, 'utf8')
	};
...
/**
 * Deploys JS to Rocket.Chat using the authentication token and ID.
 *
 * @param {string}		jsFile 	Absolute path of the JS file to deploy.
 * @param {string}   	url    	Rocket.Chat URL.
 * @param {object}   	headers Authentication headers.
 * @param {Function} 	done   	Callback.
 */
function deployJS(loggedIn, jsFile, url, headers, done) {
	let data = {
		value: fs.readFileSync(jsFile + '.js', 'utf8')
	};

	let endpoint = loggedIn ? 'settings/Custom_Script_Logged_In' : 'settings/Custom_Script_Logged_Out';
 ...
 *
 * @param {string}   cssFile Absolute path of the CSS file to deploy.
 * @param {string}   jsLoggedIn Absolute path of the JS for logged in users file to deploy.
 * @param {string}   jsLoggedOut Absolute path of the JS for logged out users file to deploy.
 * @param {Function} done    Callback.
 */
module.exports = function (cssFile, jsLoggedIn, jsLoggedOut, done) {
	parseConfig(function (url, headers) {
		url = url.replace(/\/$/, '');

		if (headers.authToken && headers.userId) {
			deployCSS(cssFile, url, headers, done, headers.minify);
			deployJS(true, jsLoggedIn, url, headers, done);
			deployJS(false, jsLoggedOut, url, headers, done);
			return;
		}
		...
		askForUsernameAndPassword(function (username, password) {
			...
			getAuthCredentials(url, data, function (error, credentials) {
				...
				writeDataToConfigFile(url, { ...credentials, minify: false });
